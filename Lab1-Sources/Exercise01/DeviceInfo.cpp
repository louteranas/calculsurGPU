  ...
  try
  {
    ...
    for (std::vector<cl::Platform>::iterator plat = platforms.begin(); plat != platforms.end(); plat++)
    {
      ...
      for (std::vector<cl::Device>::iterator dev = devices.begin(); dev != devices.end(); dev++)
      {
        ...
        // According to doc this function returns 6 bits of 0 or 1, they all need to be at one to support double precision
        cl_device_fp_config conf;
        dev->getInfo(CL_DEVICE_DOUBLE_FP_CONFIG, &conf);
        std::cout << "\t\tDevice Supports double precision: " << (63 == conf ? "True" : "False") << std::endl;
